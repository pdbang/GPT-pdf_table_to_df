...
def gpt_rows_and_cols(df_data, system_prompt: str) -> (list, list):
    ...
    return gpt_utils.get_cols_and_rows(df_data, system_prompt)

def gpt_csv_to_df(df_data, rows: list, cols: list, system_prompt: str) -> DataFrame:
    system_prompt = prompt_values_base + "\n" + system_prompt
    dico = gpt_utils.get_csv_with_cols(df_data, system_prompt, rows, cols)
